...
(function () {
  ...
  [].forEach.call(document.querySelectorAll('[data-ptabs]'), function (root) {
    ...
    // 맨 아래 탭인데 상자가 조금 더 긴 경우에는 남는 길이가 거의 전부 둥근 모서리라 탭 바로 밑에서 선이 휜다.
    // 그만큼 탭을 아래로 늘려 상자 끝에 맞춘다(글자 위치는 그대로)
    function flushBottom(i) {
      tabs.forEach(function (t) { t.style.paddingBottom = ''; });
      var tb = tabs[i].getBoundingClientRect().bottom, pb = box.getBoundingClientRect().bottom, gap = pb - tb;
      if (i === tabs.length - 1 && gap >= 1 && gap <= 64) {
        tabs[i].style.paddingBottom = (parseFloat(getComputedStyle(tabs[i]).paddingBottom) + gap) + 'px';
        gap = 0;
      }
      root.classList.toggle('is-flush-bottom', gap < 20);
    }
