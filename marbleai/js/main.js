...
(function () {
  ...
  /* 내비: 현재 구간 */
  var links = all('.nav__links a, .rail a');
  if (hasST) {
  ...
  // 링크를 누른 위치에 따라 문제 해결 탭 또는 자세히 보기의 원래 문장으로 돌아간다
  function addBack(target, from, section) {
    ...
    var sourceFold = from.closest('details.fold');
    var host = section ? section.parentNode : target.querySelector('.arch:not(.flowfig) figcaption') || target.querySelector('.fold__in');   // 흐름도 그림은 되돌아가기 버튼의 자리가 아니다
    if ((!tab && !sourceFold) || !host) return;
    all('.backlink').forEach(function (b) { b.remove(); });
    var btn = document.createElement('button');
    btn.type = 'button'; btn.className = 'backlink';
    var label = tab ? item.querySelector('h3').textContent + ' · ' + tab.querySelector('span').textContent + ' 탭' : sourceFold.querySelector('summary span').textContent;
    btn.innerHTML = icon('arrow-left') + '<span>' + label + '으로 돌아가기</span>';
    btn.addEventListener('click', function () {
      if (tab) tab.click();
      else openFold(sourceFold, true);
      btn.remove();
      (tab ? item : from).scrollIntoView({ behavior: smooth, block: 'start' });
      (tab || from).focus({ preventScroll: true });
    });
    if (section) host.insertBefore(btn, section);
    else if (host.tagName === 'FIGCAPTION') host.appendChild(btn);
    else host.insertBefore(btn, host.firstChild);
  }

  function goTo(id, from, sectionId) {
    var t = document.getElementById(id);
    if (!t) return;
    if (t.tagName === 'DETAILS') openFold(t, true);
    var section = sectionId && document.getElementById(sectionId);
    if (section && !t.contains(section)) section = null;
    if (from) addBack(t, from, section);
    // 펼친 뒤의 길이를 먼저 반영하고 나서 움직인다. 이동 중에 refresh가 끼어들면 부드러운 스크롤이 중간에 끊긴다
    refresh();
    var fig = t.querySelector('.arch:not(.flowfig)');
    if (section) (section.previousElementSibling && section.previousElementSibling.classList.contains('backlink') ? section.previousElementSibling : section).scrollIntoView({ behavior: smooth, block: 'start' });
    else if (fig) fig.scrollIntoView({ behavior: smooth, block: 'center' });   // 그림은 화면 가운데에 놓는다
    else t.scrollIntoView({ behavior: smooth, block: 'start' });
    var f = section || (t.tagName === 'DETAILS' ? t.querySelector('summary') : null);
    if (f) f.focus({ preventScroll: true });
  }
  all('.arch-node').forEach(function (g) {
    g.addEventListener('click', function () { goTo(g.dataset.target, g, g.dataset.scroll); });
    g.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); goTo(g.dataset.target, g, g.dataset.scroll); }
    });
  });
  all('a[data-open]').forEach(function (a) {
    a.addEventListener('click', function (e) { e.preventDefault(); goTo(a.dataset.open, a, a.dataset.scroll); });
  });
  // 접으면 되돌아가기 버튼도 치운다
